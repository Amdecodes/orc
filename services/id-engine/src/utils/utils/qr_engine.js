...
async function decodeWithZXing(pipeline) {
    try {
        ...
        if (decodeResult) {
            const points = decodeResult.getResultPoints();
            let bbox = null;
            ...
            return parseQRData(decodeResult.getText(), bbox);
        }
        return null;
    } catch (e) {
        return null;
...
async function decodeWithJsQR(pipeline) {
    try {
        ...
        if (code && code.data) {
            ...
            const bbox = {
                left: Math.floor(Math.min(loc.topLeftCorner.x, loc.bottomLeftCorner.x)),
                top: Math.floor(Math.min(loc.topLeftCorner.y, loc.topRightCorner.y)),
                width: Math.ceil(Math.max(loc.topRightCorner.x, loc.bottomRightCorner.x) - Math.min(loc.topLeftCorner.x, loc.bottomLeftCorner.x)),
                height: Math.ceil(Math.max(loc.bottomLeftCorner.y, loc.bottomRightCorner.y) - Math.min(loc.topLeftCorner.y, loc.topRightCorner.y))
            };
            return parseQRData(code.data, bbox);
        }
        return null;
    } catch (e) {
        return null;
 ...
 */
function parseQRData(rawData, bbox) {
    if (!rawData) return null;

    // 1. Try JSON
    try {
        const json = JSON.parse(rawData);
        return normalizeQR(json, rawData, bbox);
    } catch (e) {}
    ...
    if (rawData.includes(':')) {
        ...
        return normalizeQR(data, rawData, bbox);
    }

    return { raw: rawData, _bbox: bbox };
}

/**
 * Normalizes QR JSON keys to our standard schema
 */
function normalizeQR(data, rawPayload, bbox) {
    const raw = data || {};
