...
export async function extractThird(input) {
    const label = Buffer.isBuffer(input) ? '<buffer>' : input;
    console.log(`[Extractor:Third] Processing ${label}`);

    // For scanQR and other path-based utils we write a temp file if given a Buffer
    let imagePath = input;
    let tempPath = null;
    if (Buffer.isBuffer(input)) {
        tempPath = path.join(os.tmpdir(), `third_${Date.now()}.jpg`);
    ...
    try {
        ...
        const imgBuffer = Buffer.isBuffer(input) ? input : await fs.readFile(input);
