...
import { findBestAnchor } from "../src/anchor.js";
import { buildDynamicCrop } from "../src/crop.js";
import { recognizeName } from "../src/recognize.js";
import { getPngDimensions } from "../src/image_meta.js";

const startBatch = async () => {
  const images = fs.readdirSync("../assets/samples/front").filter(f => f.match(/^image.*\.png$/) || f.endsWith(".png"));
  ...
  for (const img of images) {
    const imagePath = path.join("../assets/samples/front", img);
    console.log(`\nProcessing: ${img}`);
